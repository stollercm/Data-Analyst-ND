...
# comment out first line when knitting html
...
# view list of variables - redundant
#names(whiteWine)
...
# plot quality histogram with count labels
...
# create new variable with quality as an ordered factor
whiteWine$quality.factor <- factor(whiteWine$quality, levels = c(3,4,5,6,7,8,9),
                                   ordered = T)

# view counts for each factor
table(whiteWine$quality.factor)

# create new ordered factor with three quality buckets
whiteWine$quality.bucket <- ifelse(whiteWine$quality < 5, "Poor", "Average")
whiteWine$quality.bucket <- ifelse(whiteWine$quality > 7, "Good", 
                                   whiteWine$quality.bucket)
whiteWine$quality.bucket <- factor(whiteWine$quality.bucket, 
                                   levels = c("Poor", "Average", "Good"), 
                                   ordered = T)

# view counts for each bucket
table(whiteWine$quality.bucket)

# plot histogram of quality bucket
...
# list variables and types again to confirm addition of new ordered factors
str(whiteWine)
```
...
# histogram of fixed acidity with narrower bins
...
# find and save 99th percentile of citric acid values (0.74)
citr99 <- quantile(whiteWine$citric.acid, 0.99)

# histogram of all citric acid values
citr.plot1 <- ggplot(data = whiteWine, aes(x = citric.acid)) +
  geom_histogram() +
  scale_x_continuous()

# histogram of all citric acid values w/ narrower bins
citr.plot2 <- ggplot(data = whiteWine, aes(x = citric.acid)) +
  geom_histogram(binwidth = 0.01) +
  scale_x_continuous(breaks = seq(0, 1.7, 0.1))

# histogram of citric acid values up to 0.75
citr.plot3 <- ggplot(data = whiteWine, aes(x = citric.acid)) +
  geom_histogram(binwidth = 0.01) +
  scale_x_continuous(lim = c(0, .75), breaks = seq(0, .8, 0.1))

# histogram: log10 transform of citric acid values up to 0.75
citr.plot4 <- ggplot(data = subset(whiteWine, citric.acid <= 0.75), 
                     aes(x = citric.acid)) +
  geom_histogram() +
  scale_x_log10() +
...
# find and save 99th percentile of resid sugar (18.8)
sug99 <- quantile(whiteWine$residual.sugar, 0.99)

# resid sugar histogram w/o top 1% of values
sug.plot4 <- ggplot(data = whiteWine, aes(x = residual.sugar)) +
  geom_histogram(binwidth = 0.1) +
  scale_x_continuous(lim = c(0, sug99), breaks = seq(0, 20, 2))

# resid sugar histogram w/ log10 transform, w/o top 1% of values
sug.plot5 <- ggplot(data = subset(whiteWine, residual.sugar < 18.8), 
                    aes(x = residual.sugar)) +
  geom_histogram() +
  scale_x_log10() +
  ggtitle("Residual Sugar (log10)")
...
# sulphates histogram w/ narrower bins
...
#alcohol histogram log 10 - somewhat more normal
...
```{r echo=FALSE}
# create new DF with transformed variables - removing outliers
...
# create new DF from 2nd one with larger group of cholorides outliers removed
...
# plot matrix from new DF with outliers removed
ggpairs(whiteWine3[1:4567, c("fixed.acidity", "volatile.acidity", "citric.acid",
                             "residual.sugar", "chlorides", 
                             "free.sulfur.dioxide", "total.sulfur.dioxide", 
                             "density", "pH", "sulphates", "alcohol", "quality",
                             "quality.factor", "quality.bucket")], 
...
# alcohol by quality w/o log10 transform - not used
...
# alcohol by quality w/ log10 transform
...
# alcohol by quality bucket w/o log10 transform - not  used
...
# alcohol by quality bucket w/ log10 transform
...
```{r echo=FALSE}
# want to zoom in on the poor and good facets from the last histogram
# distributions dwarfed  by the Average facet

# histogram of alcohol log 10 - poor quality wines only
alc.plot3 <- ggplot(data = subset(whiteWine3, whiteWine$quality < 5), 
                    aes(x = alcohol)) +
  geom_histogram() +
  scale_x_log10() +
  ggtitle('Alcohol (log10) in Poor wines')

# histogram of alcohol log 10 - good quality wines only
alc.plot4 <- ggplot(data = subset(whiteWine3, whiteWine$quality > 7), 
                    aes(x = alcohol)) +
  geom_histogram() +
  scale_x_log10() +
...
```{r echo=FALSE}
# boxplot of log10 alcohol by quality bucket
...
```{r echo=FALSE}
# boxplot of log10 alcohol by quality factor
...
```{r echo=FALSE}
# scatter of quality by alcohol w/ a trend line
...
```{r echo=FALSE}
# faceted histogram - density by quality bucket
...
# scatter of quality vs density
ggplot(data = whiteWine3, aes(x = density, y = quality)) +
  geom_jitter()

# box plot of density by quality bucket
...
```{r echo=FALSE}
# box plot of density by quality factor
...
```{r echo=FALSE}
# scatter of density vs alcohol w/ trendline
...
```{r echo=FALSE}
# scatter of density vs resid sugar w/ trendline
ggplot(data = whiteWine3, aes(x = residual.sugar, y = density)) +
  geom_point(alpha = 0.25) +
  geom_smooth(method = 'lm', color = 'red')

# scatter of density vs resid sugar log10 w/ trendline/curve
ggplot(data = whiteWine3, aes(x = residual.sugar, y = density)) +
  geom_jitter(alpha = 0.25) +
  scale_x_log10() +
...
```{r echo=FALSE}
# scatter of alcohol vs resid sugar w/ trendline
ggplot(data = whiteWine3, aes(x = residual.sugar, y = alcohol)) +
  geom_jitter(alpha = 0.25) +
  geom_smooth(method = 'lm', color = 'red')

# scatter of alcohol vs resid sugar log10 w/ trendline
ggplot(data = whiteWine3, aes(x = residual.sugar, y = alcohol)) +
  geom_jitter(alpha = 0.25) +
  scale_x_log10() +
  geom_smooth(color = 'red') +
...
```{r echo=FALSE}
# histogram of resid sugar log10 by quality bucket
ggplot(data = whiteWine3, aes(x = residual.sugar)) +
  geom_histogram(binwidth = 0.02) +
  scale_x_log10() +
  facet_wrap(~quality.bucket) 

# scatter of quality vs resid sugar log10
ggplot(data = whiteWine3, aes(x = residual.sugar, y = quality)) +
  geom_jitter() +
  scale_x_log10()

# boxplot of resid sugar by quality bucket
...
# histogram of chlorides by quality bucket
...
# scatter of quality vs chlorides
ggplot(data = whiteWine3, aes(x = chlorides, y = quality)) +
  geom_jitter()

# boxplot of chlorides by quality bucket
...
```{r echo=FALSE}
# boxplot of chlorides by quality factor
ggplot(data = whiteWine3, aes(x = quality.factor, y = chlorides)) +
  geom_boxplot()

# new DF with just the best wines - rated 9
whiteWine3Best <- subset(whiteWine3, whiteWine3$quality == 9)

# stats for chlorides in the best wines
...
```{r echo=FALSE}
# scatter of density vs alcohol by quality factor
ggplot(data = whiteWine3, 
       aes(x = alcohol, y = density, color = quality.factor)) +
  scale_colour_brewer() +
  geom_jitter()

# scatter of density vs alcohol by quality bucket
# changed colors because the blue is too hard to see in this plot
ggplot(data = whiteWine3, 
       aes(x = alcohol, y = density, color = quality.bucket)) +
...
```{r echo=FALSE}
# scatter of density vs resid sugar by quality factor
ggplot(data = whiteWine3, 
       aes(x = residual.sugar, y = density, color = quality.factor)) +
  scale_colour_brewer() +
  geom_jitter() +
  scale_x_log10()

# scatter of density vs resid sugar by quality bucket
# changed colors because the blue is too hard to see in this plot
ggplot(data = whiteWine3, 
       aes(x = residual.sugar, y = density, color = quality.bucket)) +
  scale_colour_brewer(palette="YlOrBr") +
  geom_jitter() +
  scale_x_log10()
```
...
```{r echo=FALSE}
# scatter of resid sugar vs alcohol by quality factor
ggplot(data = whiteWine3, 
       aes(x = alcohol, y = residual.sugar, color = quality.factor)) +
  scale_colour_brewer() +
  geom_jitter()

# scatter of resid sugar vs alcohol by quality bucket
# changed colors because the blue is too hard to see in this plot
ggplot(data = whiteWine3, 
       aes(x = alcohol, y = residual.sugar, color = quality.bucket)) +
  scale_colour_brewer(palette="YlOrBr") +
  geom_jitter()
```
...
```{r echo=FALSE}
# determine quartiles for alcohol variable
summary(whiteWine3$alcohol)

# create new alcohol bucket variable
whiteWine3$alcohol.bucket <- cut(whiteWine3$alcohol, 
                                 breaks = c(7.99, 9.50, 10.40, 11.40, 14.20))

# histogram of alcohol colored by bucket to show distribution
ggplot(data = whiteWine3, aes(x = alcohol, fill = alcohol.bucket)) +
  scale_colour_brewer() +
  geom_histogram(binwidth = 0.005) +
  scale_x_log10()

# scatter of density vs resid sugar by alcohol bucket
ggplot(data = whiteWine3, 
...
```{r echo=FALSE}
# scatter plots of density vs resid sugar by alcohol bucket faceted by quality
ggplot(data = whiteWine3, 
       aes(x = residual.sugar, y = density, color = alcohol.bucket)) +
  scale_colour_brewer() +
  facet_wrap(~quality.factor) +
...
# scatter plots of density vs resid sugar by alcohol bucket,
# faceted by quality bucket
ggplot(data = whiteWine3, 
       aes(x = residual.sugar, y = density, color = alcohol.bucket)) +
  scale_colour_brewer() +
  facet_wrap(~quality.bucket) +
...
```{r echo=FALSE}
# for these plots, switch facet variable with color variable

# scatter of density vs residual sugar by quality factor,
# faceted by alcohol bucket
ggplot(data = whiteWine3, 
       aes(x = residual.sugar, y = density, color = quality.factor)) +
  scale_colour_brewer(palette="YlOrBr") +
...
# scatter of density vs residual sugar by quality bucket,
# faceted by alcohol bucket
...
```{r echo=FALSE}
# scatter of density vs chlorides by quality factor
ggplot(data = whiteWine3, 
       aes(x = chlorides, y = density, color = quality.factor)) +
  scale_colour_brewer() +
  geom_jitter(alpha = 0.75)

# scatter of density vs chlorides by quality bucket
# color change for clarity
ggplot(data = whiteWine3, 
       aes(x = chlorides, y = density, color = quality.bucket)) +
  scale_colour_brewer(palette="YlOrBr") +
  geom_jitter(alpha = 0.75)
```
...
```{r echo=FALSE}
# scatter of density vs chlorides by alcohol bucket
ggplot(data = whiteWine3, 
       aes(x = chlorides, y = density, color = alcohol.bucket)) +
  scale_colour_brewer() +
  geom_jitter(alpha = .75)
```
...
```{r echo=FALSE}
# scatter of density vs chlorides by alcohol bucket, faceted by quality factor
ggplot(data = whiteWine3, 
       aes(x = chlorides, y = density, color = alcohol.bucket)) +
  scale_colour_brewer() +
  facet_wrap(~quality.factor) +
...
# scatter of density vs chlorides by alcohol bucket, faceted by quality bucket
ggplot(data = whiteWine3, 
       aes(x = chlorides, y = density, color = alcohol.bucket)) +
  scale_colour_brewer() +
  facet_wrap(~quality.bucket) +
...
```{r echo=FALSE}
# new variable summing levels of dissolved chemicals
whiteWine3$dissolved.chemicals <- whiteWine3$residual.sugar +
  whiteWine3$chlorides + (whiteWine3$total.sulfur.dioxide / 1000) + 
  whiteWine3$sulphates

# histogram of new variable with log10 transform - bimodal distribution
ggplot(data = whiteWine3, aes(x = dissolved.chemicals)) +
  geom_histogram() +
  scale_x_log10()

# scatter of density vs dissolved chem by alcohol bucket
ggplot(data = whiteWine3, 
       aes(x = dissolved.chemicals, y = density, color = alcohol.bucket)) +
  scale_colour_brewer() +
  geom_jitter(alpha = .8) +
  scale_x_log10()
```
...
```{r echo=FALSE}
# scatter of density vs dissolved chem by alcohol bucket faceted by qual factor
ggplot(data = whiteWine3, 
       aes(x = dissolved.chemicals, y = density, color = alcohol.bucket)) +
  scale_colour_brewer() +
  facet_wrap(~quality.factor) +
...
# scatter of density vs dissolved chem by alcohol bucket faceted by qual bucket
ggplot(data = whiteWine3, 
       aes(x = dissolved.chemicals, y = density, color = alcohol.bucket)) +
  scale_colour_brewer() +
  facet_wrap(~quality.bucket) +
...
# histogram of quality, colored by bucket to emphasize the groupings
...
Quality is the main feature of interest in this dataset, so it is important to understand its distribution before exploring its relationships with other variables. Here, we plot a histogram that breaks down by quality rating the 4,567 white wines that remain after removing outliers in other variables. The quality ratings are distributed fairly normally, with a range of 3 to 9, a mean of approximately 5.906 and a median of 6. We can gain some symmetry by grouping the wines into Poor, Average, and Good quality ratings. Clearly, the vast majority of the wines fall into the Average (5-7) category; in fact, about 92.6% of the wines qualify as Average by this grouping. The remaining wines are split almost evenly between Poor (3-4) and Good (8-9).

### Plot Two
```{r echo=FALSE, Plot_Two}
# scatterplot of density vs alcohol colored by quality rating
ggplot(data = whiteWine3, 
       aes(x = alcohol, y = density, color = quality.factor)) +
  scale_colour_brewer(name="Quality", palette="YlOrBr") +
  geom_jitter(alpha = 1) +
...
# scatterplot of density vs alcohol colored AND faceted by quality rating
ggplot(data = whiteWine3, 
       aes(x = alcohol, y = density, color = quality.factor)) +
  scale_colour_brewer(name="Quality", palette="YlOrBr") +
...
The relationship between density and alcohol is the strongest bivariate relationship in the dataset. There is a strong negative correlation (-0.816) between these two variables. This makes sense from a chemistry standpoint; alcohol is less dense than water, and thus higher levels of alcohol will tend to decrease the density of the wine. The variability in this relationship would be explained by the differing levels of dissolved chemicals in the wines. 

Because quality is the feature of interest in this analysis, we examine the interplay among all three of these variables. Here we see two plots that help us visualize these relationships: the first one plots density levels against alcohol content, colored by quality rating. The second does the same, but shows one scatterplot for each quality rating for clarity. It's useful to facet the plot in this way in order to see more specific trends that are obscured by some overplotting in the first plot. From that first plot, we get a sense that the more highly rated white wines tend to be stronger and less dense than many other wines, though there are some strong wines with low densities that have lower ratings as well. The faceted plot shows this well, as we can see more plainly the distribution of alcohol levels for each quality ratings.

### Plot Three
```{r echo=FALSE, Plot_Three}
# boxplot of density by alcohol bucket
ggplot(data = whiteWine3, 
...
# scatterplot of density vs dissolved chemicals by alcohol bucket
ggplot(data = whiteWine3, 
       aes(x = dissolved.chemicals, y = density, color = alcohol.bucket)) +
  scale_colour_brewer(name="Alcohol Bucket", palette="YlOrBr") +
  geom_jitter(alpha = .8) +
  scale_x_log10() +
...
# scatterplot of density vs dissolved chemicals by alcohol bucket faceted by quality
ggplot(data = whiteWine3, 
       aes(x = dissolved.chemicals, y = density, color = alcohol.bucket)) +
...
For future uses of this data, it would be interesting to combine it with the corresponding data for red wine, building categorical variables and performing transformations as needed. This would allow for the generation of visualizations that would help in comparing how these characteristics impact quality in red and white wines.
